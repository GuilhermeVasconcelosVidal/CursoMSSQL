...
--TABEAL DE FUNCIONARIO VERIFICAÇÃO DE ADMISSÃO E DEMISSÃO
...
--SET @DATA_PAGTO='2017-04-05'
  ...
  --LENDO REGISTROS
	...
	SELECT A.PCT FROM PARAM_INSS A
				 ...
				 AND A.VALOR_ATE
				 AND @MES_REF BETWEEN MONTH(A.VIGENCIA_INI) 
				 AND MONTH(A.VIGENCIA_FIM)
				 AND @ANO_REF BETWEEN YEAR (A.VIGENCIA_INI) 
				 AND YEAR(A.VIGENCIA_FIM)
  ...
  --SELECT * FROM PARAM_IRRF
	...
	--PROXIMO REGISTRO CALC_INSS
	  FETCH NEXT FROM CALC_INSS
	  INTO @PCT_INSS
	    END --FIM DO WHILE CALC_INSS
	
	--FECHANDO CURSOR CALC_INSS	 
	  CLOSE CALC_INSS
...
--SELECT * FROM PARAR_IRRF

--SELECT * FROM FOLHA_PAGTO
--TESTANDO PROC
--PARAMETROS @COD_EMPRESA,@MES_REF,@ANO_REF,@DATA_PAGTO
  EXEC PROC_FOLHA 1,2,2018,'05-02-2018'
