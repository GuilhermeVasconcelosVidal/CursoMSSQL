...
--PROCEDURE QUE GERA ORDENS DE PRODUÇAO COM BASE EM PEDIDOS DE VENDAS
  ...
  AS 
  BEGIN
--SE ATIVADA ESTA INSTRUÇAO TEM POR OBJETIVO INFORMAR
--EM TELA  A QUANTIDADE  DE LINHAS AFETADAS PELA TRANSFORMAÇAO
...
--VERIFICANDO SE EXISTE PEDIDOS ABERTO  PARA MES E ANO SELECIONADO 
  ...
  END
    ELSE 
	    BEGIN
			...
			EOMONTH(@ANO+'-'+'-01') AS DATA_FIM,'A' --EOMONTH TRAZ O ULTIMO DIA DO MES DA DATA PARAM
			FROM PED_VENDAS A
			INNER JOIN PED_VENDAS_ITENS B
			ON A.NUM_PEDIDO=B.NUM_PEDIDO
			WHERE A.COD_EMPRESA=@COD_EMPRESA
			AND A.SITUACAO='A' --APENAS PEDIDO ME ABERTO
			AND MONTH (A.DATA_ENTREGA)=@MES
			AND YEAR(A.DATA_ENTREGA )=@ANO
  ...
  PRINT'PEDIDOS SITUAÇAO ATUALIZADA';
  END --FINAL ELSE
...
--TESTES FINAIS 
  IF @ERRO_INTERNO=1
      BEGIN
	   ROLLBACK
	   RAISERROR('NAO EXISTE MATERIAIS PARA	PNANEJ.ORDEM',--MESSAGE TEXT.
	                10, --SEVERITY.
					1 --STATE.
					);
...
--OBJETIVO GERAR ORDENS DE PRODUÇAO DE ACORDO COM DEMANDA DE VENDAS
--DROP PROCEDURE PROC_PLAN_ORDEM
--SELECT * FROM ORDEM_PROD
--SELECT * FROM PED_VENDAS