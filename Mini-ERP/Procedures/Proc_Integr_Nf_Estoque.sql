...
FETCH NEXT FROM LE_NFE_VENDA
 ...
 --DECLARANDO CURSOR PARA LER ESTOQUE COM MATERIAIS DA NOTA PARA BAIXA EM ESTOQUE
  ...
  --ATRIBUINDO VALOR NAS VARIAVEIS
		  ...
		  WHILE  @@FETCH_STATUS= 0
		      BEGIN
--VARIFICA합ES DE TROCA DE MATERIAL
			   ...
			   BEGIN
				...
				SET	@SALDO=@QTD;
			    END
--VERIFICA합ES DE SALDO
              IF @SALDO<=0
			  BEGIN 
			  SET @QTD_ATEND=0
			  END
--ESTRUTURA DE VERIFICA플O DE QUANTIDADE PEDIDO SALDO LOTE E SALDO PEDIDO
		...
		--ATRIBUINDO VALOR VARIAVEL
		  ...
		  END
        --LENDO A PROXIMA LINHA DO CURSOR
		  FETCH NEXT FROM INTEGRA_NFE_VENDA
		  INTO @COD_MAT, @QTD_LOTE, @LOTE
		  END
        ...
        --LENDO PROXIMA LINHA DO CURSOR
		  ...
		  FETCH NEXT FROM LE_NFE_VENDA
		        INTO @NUM_NF,@TIP_NF,@COD_MAT,@QTD
		--ATUALIZANDO NOTA_FISCAL COMO INTEGRADA
        ...
        --FECHANDO E DESALOCANDO CURSOR
		  ...
		  END TRY -- END TRY
		    BEGIN CATCH 
				...
				IF(SELECT CURSOR_STATUS ('global','LE_NFE_VENDAS')) = 1
				BEGIN
				    CLOSE LE_NFE_VENDA
					DEALLOCATE LE_NFE_VENDA
				END
...
--ULTIMAS VERIFICA합ES PARA COMMIT OU ROLLBACK
--LABEL DO GOTO
  ERRO4:
     --FECHANDO CURSORES
...
EXECUTE PROC_INTEGR_NF_ESTOQUE 1,1,'2018-01-01' --ENTRADA
EXECUTE PROC_INTEGR_NF_ESTOQUE 1,2,'2018-01-01' --ENTRADA
EXECUTE PROC_INTEGR_NF_ESTOQUE 1,3,'2018-01-01' --ENTRADA
EXECUTE PROC_INTEGR_NF_ESTOQUE 1,4,'2018-01-01' --ENTRADA
EXECUTE PROC_INTEGR_NF_ESTOQUE 1,5,'2018-01-01' --ENTRADA