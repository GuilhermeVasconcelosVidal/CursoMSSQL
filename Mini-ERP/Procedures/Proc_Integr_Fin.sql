...
--FASE 7 INTEGRAÇÃO NFE COM FINANCEIRO CAP E CRE
...
GO
  ...
  BEGIN
   BEGIN TRANSACTION
   BEGIN TRY
--ABRINDO PRIMEIRO IF VARIFICAR SE EXISTE NOTA PARA INTEGRAR
  IF (SELECT COUNT(*) 
      FROM NOTA_FISCAL 
  ...
  --ABRINDO CURSOR
	...
	BEGIN
	  --IF PARA INTEGRAR NOTAS  DE VENDAS SAIDAS
	   IF @TIP_NF='S'
	    BEGIN
		  INSERT INTO CONTAS_RECEBER VALUES (@COD_EMPRESA ,@ID_CLIFOR,@NUM_NF,@PARC,
		                                     @VENCIMENTO,NULL,@VALOR_PARC);
		  SELECT 'DOCTO VENDAS',@NUM_NF, 'INTEGRADO COM SUCESSO', 'PARCELA ',@PARC, 'VALOR ',@VALOR_PARC
		END
		--IF PARA INTEGRAR NOTAS DE COMPRAS ENTRADAS
		ELSE IF @TIP_NF='E'
		BEGIN
		  INSERT INTO CONTAS_PAGAR VALUES (@COD_EMPRESA ,@ID_CLIFOR,@NUM_NF,@PARC,
		                                     @VENCIMENTO,NULL,@VALOR_PARC);
